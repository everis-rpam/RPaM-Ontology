...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p><xd:b>Updated on:</xd:b> Nov 14, 2019</xd:p>
    ...
    <xsl:template match="/">
        <xsl:for-each select="/JSON/mandate">
            <PAM:Mandate>  
                <xsl:call-template name="create_LifeSpanPeriod">
                    <xsl:with-param name="startDate" select="valid_from"/>
                    <xsl:with-param name="endDate" select="valid_to"/>
                </xsl:call-template>   
        ...
        <xsl:for-each select="/JSON/child::*[local-name()=$person_type]">
            <PAM:NaturalPerson>
                ...
                <PAM:NaturalPersonPerson>
                    ...
                    <xsl:call-template name="create_element">
                        <xsl:with-param name="element_name">xhb:OtherName</xsl:with-param>
                        <xsl:with-param name="value"><xsl:value-of select="common_name"/></xsl:with-param>
                    </xsl:call-template>
                    <xsl:call-template name="create_element">
                        <xsl:with-param name="element_name">xhb:BirthDate</xsl:with-param>
                        <xsl:with-param name="value"><xsl:value-of select="substring-before(date_of_birth, ' 00:00:00')"/></xsl:with-param>
                    </xsl:call-template>
                </PAM:NaturalPersonPerson>
            </PAM:NaturalPerson>
        </xsl:for-each>
    ...
    <xsl:template name="create_Mandator">
        <PAM:Mandator>
            ...
            <PAM:Power>
                <xhb:AdditionalInformation><xsl:value-of select="concat(/JSON/mandate_information/description, /JSON/mandate_information/description_detail)"/></xhb:AdditionalInformation>
    ...
    <xd:doc>
        <xd:desc><xd:p>Create LifeSpanPeriod element (if at least one of the parameters exists).</xd:p></xd:desc>
        <xd:param name="startDate"/>
        <xd:param name="endDate"/>
    </xd:doc>
    <xsl:template name="create_LifeSpanPeriod">
        <xsl:param name="startDate"/>
        <xsl:param name="endDate"/>
        
        <xsl:if test="(not(empty($startDate)) and $startDate!='') or (not(empty($endDate)) and $endDate!='')">
           <PAM:LifeSpanPeriod>
               <xsl:call-template name="create_element">
                   <xsl:with-param name="element_name">xhb:StartDate</xsl:with-param>
                   <xsl:with-param name="value"><xsl:value-of select="$startDate"/></xsl:with-param>
               </xsl:call-template>
               
               <xsl:call-template name="create_element">
                   <xsl:with-param name="element_name">xhb:EndDate</xsl:with-param>
                   <xsl:with-param name="value"><xsl:value-of select="$endDate"/></xsl:with-param>
               </xsl:call-template>
           </PAM:LifeSpanPeriod> 
        </xsl:if>
    ...
    <xsl:template name="create_element">
        ...
        <xsl:if test="not(empty($value)) and $value!='' and $value!='null'">

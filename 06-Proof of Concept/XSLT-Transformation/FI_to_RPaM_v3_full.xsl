...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
            ...
            <PAM:NaturalPerson>
                ...
                <PAM:NaturalPersonPerson>
                    <xsl:call-template name="create_element">
                        ...
                        <xsl:with-param name="value"><xsl:value-of select="firstname"/></xsl:with-param>
                    </xsl:call-template>
                    <xsl:call-template name="create_element">
    ...
    <xsl:template name="create_Mandator">
        <PAM:Mandator>
            ...
            <PAM:Power>
                <xsl:call-template name="create_element">
                    <xsl:with-param name="element_name">xhb:AdditionalInformation</xsl:with-param>
                    <xsl:with-param name="value"><xsl:value-of select="/Mandate/Power/definition"/></xsl:with-param>
                </xsl:call-template>
                <xhb:RpaMPowerCode>BE_TOP.BE_MESSAGING_WITH_AUTHORITIES.AC_MANAGING.OB_ELECTRONIC_MESSAGES.OT_NOTIFICATION</xhb:RpaMPowerCode>
        ...
        <xsl:if test="(not(empty($startDate)) and $startDate!='') or (not(empty($endDate)) and $endDate!='')">
            <PAM:LifeSpanPeriod>
                <xsl:call-template name="create_element">
                    ...
                    <xsl:with-param name="value"><xsl:value-of select="concat(substring($startDate, 7, 4), '-', substring($startDate, 4, 2), '-', substring($startDate, 1, 2))"/></xsl:with-param>
                </xsl:call-template>
                
                <xsl:call-template name="create_element">
                    <xsl:with-param name="element_name">xhb:EndDate</xsl:with-param>
                    <xsl:with-param name="value"><xsl:value-of select="concat(substring($endDate, 7, 4), '-', substring($endDate, 4, 2), '-', substring($endDate, 1, 2))"/></xsl:with-param>
                </xsl:call-template>
            </PAM:LifeSpanPeriod> 
        </xsl:if>
    </xsl:template>

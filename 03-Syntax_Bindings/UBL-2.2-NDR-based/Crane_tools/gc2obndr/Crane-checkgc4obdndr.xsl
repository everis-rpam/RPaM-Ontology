...
<xs:doc info="$Id: Crane-checkgc4obdndr.xsl,v 1.33 2018/02/19 19:20:25 admin Exp $"
...
<xsl:template name="gu:makeReport">
  <xsl:param name="common" as="xsd:boolean"/>
  <table>
  <title>
...
<xsl:template name="gu:findChanges">
  ...
  <xsl:for-each select="$abies,($gu:deletedABIEs/.)">
    ...
    <xsl:choose>
      ...
      <xsl:when
            test="not(key('gu:bie-by-class-and-name-and-type',
